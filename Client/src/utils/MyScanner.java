 ...
 * regular expressions.
 *
 * <p>A {@code MyScanner} breaks its input into tokens using a
 ...
 * by a MyScanner is as recognized by {@link Character#isWhitespace(char)
 ...
 * readable's {@link Readable#read read()} method throws an {@link
 * IOException} then the MyScanner assumes that the end of the input
 * has been reached.  The most recent {@code IOException} thrown by the
 * underlying readable can be retrieved via the {@link #ioException} method.
 *
 * <p>When a {@code MyScanner} is closed, it will close its input source
 * if the source implements the {@link Closeable} interface.
 *
 * <p>A {@code MyScanner} is not safe for multithreaded use without
 * external synchronization.
 *
 * <p>Unless otherwise mentioned, passing a {@code null} parameter into
 * any method of a {@code MyScanner} will cause a
 ...
 * DecimalFormat DecimalFormat} object, {@code df}, and its and
 ...
 *            {@link Character#isDigit Character.isDigit}{@code (c)}
 *                        returns&nbsp;true
 *
 *   <dt><i>Non0Digit</i>:
 ...
 * @since   1.5
 */
public final class MyScanner implements Iterator<String>, Closeable {
    ...
    private int savedMyScannerPosition = -1;
    ...
    // Number of times this MyScanner's state has been modified.
    ...
    /**
     * Constructs a {@code MyScanner} that returns values scanned
    ...
    /**
     * Constructs a new {@code MyScanner} that produces values scanned
     ...
     * @since  10
     */
    public MyScanner(File source, Charset charset) throws IOException {
        this(Objects.requireNonNull(source), charset.newDecoder());
    }

    private MyScanner(File source, CharsetDecoder dec)
            throws FileNotFoundException
    {
     ...
     * @since   1.7
     */
    public MyScanner(Path source, String charsetName) throws IOException {
     ...
     * @since   10
     */
    public MyScanner(Path source, Charset charset)  throws IOException {
     ...
     * @since 10
     */
    public MyScanner(ReadableByteChannel source, Charset charset) {
        this(makeReadable(Objects.requireNonNull(source, "source"), charset),
                WHITESPACE_PATTERN);
    }
    ...
    private void saveState() {
        savedMyScannerPosition = position;
    }

    private void revertState() {
        this.position = savedMyScannerPosition;
        savedMyScannerPosition = -1;
        skipped = false;
    }
    ...
    private boolean makeSpace() {
        clearCaches();
        int offset = savedMyScannerPosition == -1 ?
                position : savedMyScannerPosition;
        buf.position(offset);
    ...
    private void translateSavedIndexes(int offset) {
        if (savedMyScannerPosition != -1)
            savedMyScannerPosition -= offset;
    }
     ...
     * {@code MyScanner}'s underlying {@code Readable}. This method
    ...
    /**
     * Returns the {@code Pattern} this {@code MyScanner} is currently
     ...
     * by this MyScanner. This method throws {@code IllegalStateException}
     * if no match has been performed, or if the last match was
     * not successful.
     *
     * <p>The various {@code next} methods of {@code MyScanner}
    ...
    /**
     * <p>Returns the string representation of this {@code MyScanner}. The
     * string representation of a {@code MyScanner} contains information
     ...
     * separator, then {@code null} is returned and the MyScanner's
     ...
     *
     * <p>A MyScanner will never search more than {@code horizon} code
     ...
     *
     * <p> An invocation of this method of the form
     * {@code MyScanner.reset()} behaves in exactly the same way as the
     * invocation
     *
     * <blockquote><pre>{@code
    ...
    /**
     * Returns a stream of delimiter-separated tokens from this MyScanner. The
     * stream contains the same tokens that would be returned, starting from
     * this MyScanner's current state, by calling the {@link #next} method
     ...
     * current state of this MyScanner. Subsequent calls to any methods on this MyScanner
     ...
     *
     * <p>After stream pipeline execution completes, this MyScanner is left in an indeterminate
     * state and cannot be reused.
     *
     * <p>If this MyScanner contains a resource that must be released, this MyScanner
     * should be closed, either by calling its {@link #close} method, or by
     * closing the returned stream. Closing the stream will close the underlying MyScanner.
     * {@code IllegalStateException} is thrown if the MyScanner has been closed when this
     * method is called, or if this MyScanner is closed during stream pipeline execution.
     *
     ...
     *
     * <pre>{@code
     * List<String> result = new MyScanner("abc,def,,ghi")
     *     .useDelimiter(",")
     *     .tokens()
    ...
    /**
     * Returns a stream of match results from this MyScanner. The stream
     ...
     *
     * <pre>{@code
     * try (MyScanner sc = new MyScanner(Path.of("input.txt"))) {
     ...
     *
     * <pre>{@code
     *     MyScanner.findAll(Pattern.compile(patString))
     * }</pre>
     *
